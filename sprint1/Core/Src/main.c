...
void EncoderTask(void *args) {
	...
	for (;;) {
		encoderPosition = LL_TIM_GetCounter(TIM4);
...
void MotorTask(void *args) {
	// 6-Step Open-Loop Sequenz
	const uint8_t stepSequence[6] = { 0b001, 0b101, 0b100, 0b110, 0b010, 0b011 };
	uint8_t step = 0;
	uint32_t delay_ms = 50; // Startgeschwindigkeit

	for (;;) {
		SetMotorPhase(stepSequence[step]); // PWM-Ausgänge setzen
...
void SetMotorPhase(uint8_t bits)
{
    // Enable Treiber
    LL_GPIO_SetOutputPin(EN_PORT, EN_PIN);

    // Bit 0 -> Phase C
    if (bits & 0x01) LL_GPIO_SetOutputPin(IN3_PORT, IN3_PIN);
    else LL_GPIO_ResetOutputPin(IN3_PORT, IN3_PIN);

    // Bit 1 -> Phase B
    if (bits & 0x02) LL_GPIO_SetOutputPin(IN2_PORT, IN2_PIN);
    else LL_GPIO_ResetOutputPin(IN2_PORT, IN2_PIN);

    // Bit 2 -> Phase A
    if (bits & 0x04) LL_GPIO_SetOutputPin(IN1_PORT, IN1_PIN);
    else LL_GPIO_ResetOutputPin(IN1_PORT, IN1_PIN);
}
...
int main(void) {
	...
	xTaskCreate(EncoderTask, "EncoderTask", 256, NULL, 4, NULL);
...
static void MX_GPIO_Init(void) {
	...
	/**/
	LL_GPIO_SetOutputPin(GPIOC, LCD_RESET_Pin | LCD_DC_Pin);
	...
	/**/
	LL_GPIO_SetPinPull(ENC_SW_GPIO_Port, ENC_SW_Pin, LL_GPIO_PULL_DOWN);
