...
int main(void) {
	...
	printf("\r\n\r\n");
	printf("=====================================\r\n");
	printf("  STM32F4 LCD + Encoder System\r\n");
	printf("=====================================\r\n");
	printf("System Clock: %lu Hz\r\n", SystemCoreClock);
	printf("FreeRTOS: Starting tasks...\r\n");
	/* USER CODE END 2 */
	...
	ret = xTaskCreate(display_task, "Display", 2048, NULL, 3, NULL);
	if (ret != pdPASS) {
		printf("ERROR: Failed to create Display task\r\n");
		Error_Handler();
	}

	ret = xTaskCreate(encoder_task, "Encoder", 256, NULL, 2, NULL);
	if (ret != pdPASS) {
		printf("ERROR: Failed to create Encoder task\r\n");
		Error_Handler();
	}
...
static void MX_TIM4_Init(void) {
	...
	LL_TIM_SetAutoReload(TIM4, 65535);       // Set ARR
	LL_TIM_SetCounter(TIM4, 32768);          // ← Must be 32768, NOT 0!
	LL_TIM_EnableCounter(TIM4);              // Start timer
